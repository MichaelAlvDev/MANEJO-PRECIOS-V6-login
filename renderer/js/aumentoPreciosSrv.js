//console.log('aumentoPreciosSrv.js cargado');
...
document.getElementById('ajusteForm').addEventListener('submit', async e => {
  ...
  try {
    const res = await fetch('http://localhost:3000/api/servicios/aumentar', {
      method: 'POST',
...
document.getElementById('basadoForm').addEventListener('submit', async e => {
  ...
  try {
    const res = await fetch(
      'http://localhost:3000/api/servicios/precio3-basado-en-2',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
...
async function cargarInstancias() {
  const res = await fetch('http://localhost:3000/api/servicios/instancias');
